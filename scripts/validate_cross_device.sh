...
c_info "1. claude.ai (or Claude Desktop) → Settings → Connectors → Add custom connector:"
c_info "     Name: anything   URL: https://$TEST_APP_NAME.fly.dev/mcp   (note the /mcp)"
c_info "     Leave Client ID / Client Secret BLANK — the AS self-registers (DCR)."
c_info "2. Click Connect. It redirects to a LOGIN PAGE — enter the passphrase THERE"
c_info "   (it is NOT a connector field; it's the OAuth login prompt):"
c_info "     $PASSPHRASE"
c_info "3. Then in that client: \"save a memory: cross-device validation OK\" →"
c_info "   \"search your memory for cross-device validation\". If it round-trips, PASS."
c_info "   (If your prod mnemon connector is also enabled, disable it first so the"
c_info "    save/search routes to THIS test app — both expose the same tool names.)"
echo "══════════════════════════════════════════════════════════════════════"
